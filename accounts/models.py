...
# Use string notation for FKs to other apps to prevent potential circular imports
# from persons.models import Employee # Use string notation below
# from projects.models import Project # Use string notation below
import uuid
...
class ChartOfAccounts(models.Model):
    ...
    class AccountSubType(models.TextChoices):
        # Assets
        BANK = 'BANK', _('Bank'); CASH = 'CASH', _('Cash / Petty Cash') # Added CASH
        ACCOUNTS_RECEIVABLE = 'ACCOUNTS_RECEIVABLE', _('Accounts Receivable (A/R)')
        EMPLOYEE_ADVANCES = 'EMPLOYEE_ADVANCES', _('Employee Advances (Asset)') # Added subtype for tracking asset value
        INVENTORY = 'INVENTORY', _('Inventory')
        PREPAID_EXPENSES = 'PREPAID_EXPENSES', _('Prepaid Expenses')
        OTHER_CURRENT_ASSET = 'OTHER_CURRENT_ASSET', _('Other Current Asset') # e.g., Digital Wallets could go here or have specific types
        FIXED_ASSET = 'FIXED_ASSET', _('Fixed Asset')
        # Liabilities
        ACCOUNTS_PAYABLE = 'ACCOUNTS_PAYABLE', _('Accounts Payable (A/P)')
        CREDIT_CARD_PAYABLE = 'CREDIT_CARD_PAYABLE', _('Credit Card Payable') # Added subtype
        ACCRUED_LIABILITIES = 'ACCRUED_LIABILITIES', _('Accrued Liabilities')
        UNEARNED_REVENUE = 'UNEARNED_REVENUE', _('Unearned Revenue')
        CURRENT_LIABILITY = 'CURRENT_LIABILITY', _('Other Current Liability') # Renamed for clarity
...
class DisbursementVoucher(AuditableModel):
    ...
    bank_account = models.ForeignKey( # Account funds are disbursed from
        ...
        # Allow BANK or CASH subtypes - *** FIXED NameError HERE ***
...
# --- EmployeeAdvance Model Moved Here ---
class EmployeeAdvance(AuditableModel):
    ...
    employee = models.ForeignKey(
        'persons.Employee', # Use string notation
    ...
    project = models.ForeignKey(
        'projects.Project', # Use string notation
        on_delete=models.SET_NULL, null=True, blank=True,
        verbose_name=_("Related Project"), related_name="employee_advances"
    )
    ...
    asset_account = models.ForeignKey(
         ...
         null=True, # Should ideally be set, maybe on save? Or allow blank=True?
    ...
    def balance_remaining(self):
        # Fix TypeError by handling None for amount_issued
